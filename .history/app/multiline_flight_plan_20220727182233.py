...
def write_kml(waypts):
    ...
    for prev_waypt in waypts:
        ...
        #    linecoords = [(waypt[2], waypt[3]), (next_waypt[2], next_waypt[3])]    
        #print(linecoords)
        mpnt = multipnt.newpoint(name='Point: {0},{1}'.format(waypt[2],waypt[3]))    #coords=[(waypt[1], waypt[2])])
        mpnt.coords = [(waypt[2],waypt[3])]
        multipnt.style.balloonstyle.text = 'Strip ' + str(waypt[2]) +' Lon ' + str(waypt[2]) + ', Lat ' + str(waypt[3]) + ', Alt ' + str(waypt[4])
