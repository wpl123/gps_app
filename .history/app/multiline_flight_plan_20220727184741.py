...
def write_kml(waypts):
    ...
    lolabels = kml.newfolder(name="Flight Plan") 
    ...
    for prev_waypt in waypts:
        ...
        # TODO: While not the last record 
        waypt      = next(iterators)
        next_waypt = next(iterators)
        linecoords = [(prev_waypt[2], prev_waypt[3]), (waypt[2], waypt[3])]
        linecoords = [(waypt[2], waypt[3]), (next_waypt[2], next_waypt[3])]
        
        mpnt = multipnt.newpoint(name='Point: {0}'.format(prev_waypt[0]))    #coords=[(waypt[1], waypt[2])])
        mpnt.coords = [(prev_waypt[2],prev_waypt[3])]
        lo = lolabels.newpoint(name='Strip: ' + str(prev_waypt[1]), coords=[(prev_waypt[2],prev_waypt[3])])
        #lo.iconstyle.icon.href = "http://maps.google.com/mapfiles/kml/shapes/placemark_circle.png"
        #multipnt.style.balloonstyle.text = 'Strip ' + str(prev_waypt[1]) +' Lon ' + str(prev_waypt[2]) + ', Lat ' + str(prev_waypt[3]) + ', Alt ' + str(prev_waypt[4])
        multipnt.newpoint(coords=[(waypt[2], waypt[3])])
